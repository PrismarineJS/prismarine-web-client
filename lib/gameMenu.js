...
class GameMenu extends LitElement {
  constructor () {
    super()
    this.inMenu = false
    if (optionsMenu === null) console.log('Options menu is null... idk why')
    optionsMenu.init()
  }
  ...
  render () {
    ...
    <div class="menu-box">
      <h2 class="title">Game Menu</h2>
        ...
        <div class="spacev"></div>
        <legacy-button btn-width="100%" @click=${() => { optionsMenu.enableOptionsMenu() }}>Options</legacy-button>
        <div class="spacev"></div>
  ...
  init (renderer, optionsMenu) {
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.keyCode === 27 || e.key === 'Escape' || e.key === 'Esc') {
        if (self.inMenu && (optionsMenu == null || !optionsMenu.inMenu)) {

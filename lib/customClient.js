...
const createSerializer = require('minecraft-protocol/src/transforms/serializer').createSerializer
const createDeserializer = require('minecraft-protocol/src/transforms/serializer').createDeserializer
const createCipher = require('minecraft-protocol/src/transforms/encryption').createCipher
const createDecipher = require('minecraft-protocol/src/transforms/encryption').createDecipher

const closeTimeout = 30 * 1000

class Client extends EventEmitter {
  constructor (isServer, version, customPackets, hideErrors = false, /** @type {{ sendData, receiverSetup }|undefined} */customCommunication = undefined) {
    super()
    // todo remove
    console.log('using patched client')
    if (customPackets === true) {
      window.serverDataChannel ??= {}
      customCommunication = {
        sendData: (data) => {
          window.serverDataChannel[isServer ? 'emitClient' : 'emitServer'](data)
        },
        receiverSetup (processData) {
          window.serverDataChannel[isServer ? 'emitServer' : 'emitClient'] = (data) => {
            processData(data)
          }
        }
      }
  ...
  setSerializer (state) {
    ...
    if (this.customCommunication) {
      this.customCommunication.receiverSetup((/** @type {{name, params}} */parsed) => {
  ...
  connect (port, host) {
    ...
    require('minecraft-protocol/src/client/tcp_dns')(this, options)

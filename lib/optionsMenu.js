...
class OptionsMenu extends LitElement {
  ...
  setSens (num) {
    console.log(num)
    const prep = Math.floor(typeof (num) === 'number' ? num : num.replace(/[^\d.-]/g, ''))
    this.shadowRoot.getElementById('sensitivity').value = prep + '%'
    window.settings.mouseSens = prep / 100 * 0.005
  }
  ...
  render () {
    ...
    <div class="menu-box">
      <h2 class="title">Options Menu</h2>
        ...
        <div class="wrapper">
          <legacy-button btn-width="100%" @click=${e => { this.setSens(((window.settings.mouseSens / 0.005) * 100) + 1) }}>++</legacy-button>
          <legacy-text-field field-width="100%" field-id="sensitivity" id="sensitivity" field-value="${(window.settings.mouseSens * 100) / 0.005}%" @input=${e => { this.setSens(e.target.value) }}></legacy-text-field>
          <legacy-button btn-width="100%" @click=${e => { this.setSens(((window.settings.mouseSens / 0.005) * 100) - 1) }}>--</legacy-button>
        </div>
    </div>
    `
  ...
  init (renderer) {
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.key === 'z' || e.key === 'z') {
        ...
        if (this.inMenu) {
          this.disableOptionsMenu(renderer)
        } else {
          this.enableOptionsMenu()
        }
      }
      if (e.keyCode === 27 || e.key === 'Escape' || e.key === 'Esc') {
        if (this.inMenu) this.disableOptionsMenu(renderer)
      }
    })
  }
}

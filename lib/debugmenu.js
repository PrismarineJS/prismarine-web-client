...
class DebugMenuEntry extends LitElement {
  static get styles () {
    return css`
        p {
            margin: 0px;
  ...
  render () {
    return html`
      <p style="display:${this.hidden ? 'none' : 'block'}">${this.text}</p>${this.breaking ? html`<br>` : ''}
    `
  }
  ...
  init (text, hidden, breaking, parent) {
    this.text = text
    this.hidden = hidden
...
class DebugMenu extends LitElement {
  ...
  render () {
    ...
    <div id="debugmenu-wrapper" class="debugmenu-wrapper">
      <div class="debugmenu" id="debugmenu" style="display: none;">
        ...
        ${Object.entries(this.debugEntries).map(([id, entry]) => entry)}
      </div>
    </div>
    `
  }
  ...
  addEntry (text, breaking = false) {
    ...
    this.debugEntries[id] = new DebugMenuEntry()
    this.debugEntries[id].init(text, false, breaking, this)
   ...
   */
  updateEntry (id, text) {
    const entry = this.debugEntries[id]
...
window.customElements.define('debug-entry', DebugMenuEntry)

...
class DebugMenu extends LitElement {
   ...
   * @return {string} Debug entry id - keep it somewhere to update entry contents later!
   */
  addEntry (text, breaking = false) {
    const generateId = () => {
      const result = 'xxxxxxxx'.replace(/[x]/g, c => {
        const r = Math.random() * 16 | 0
        return r.toString(16)
      })
      return !Object.keys(this.debugEntries).includes(result) ? result : generateId()
    }
   ...
   *
   * @param  {string} id - Debug entry id (returned with addEntry())
   * @param  {string} text - Text to render

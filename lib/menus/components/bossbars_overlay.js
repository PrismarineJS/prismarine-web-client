...
class BossBar extends LitElement {
  ...
  setTitle (bar) {
    ...
    else if (bar._title.translate) {
      if (bar._title.translate === 'entity.minecraft.ender_dragon') {
        this.title = 'Ender Dragon'
      } else if (bar._title.translate === 'entity.minecraft.wither') {
        this.title = 'Wither'
      } else {
        this.title = 'Unknown Entity'
      }
    }
  }
...
class BossBars extends LitElement {
  ...
  render () {
    return html`<div class="bossBars" id="bossBars">${
      Array.from(this.bossBars).map(([key, value]) =>
              html`<pmui-bossbar bar='${JSON.stringify(value)}'></pmui-bossbar>`
    )}</div>`
  }

  init () {
    this.bot.on('bossBarCreated', async (bossBar) => {
      this.bossBars.set(bossBar.entityUUID, bossBar)
      this.requestUpdate()
    })

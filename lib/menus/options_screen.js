...
class OptionsScreen extends CommonOptionsScreen {
  ...
  constructor () {
    ...
    this.defineOptions({
      mouseSensX: { defaultValue: 50, convertFn: (v) => Math.floor(Number(v)) },
      mouseSensY: { defaultValue: 50, convertFn: (v) => Math.floor(Number(v)) },
      chatWidth: { defaultValue: 320, convertFn: (v) => Number(v) },
      chatHeight: { defaultValue: 180, convertFn: (v) => Number(v) },
      chatScale: { defaultValue: 100, convertFn: (v) => Number(v) },
      sound: { defaultValue: 50, convertFn: (v) => Number(v) },
      renderDistance: { defaultValue: 6, convertFn: (v) => Number(v) },
      fov: { defaultValue: 75, convertFn: (v) => Number(v) },
      guiScale: { defaultValue: 3, convertFn: (v) => Number(v) },

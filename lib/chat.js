...
class ChatBox extends LitElement {
  ...
  init (client, renderer) {
    ...
    function enableChat (isCommand) {
      ...
      Array.from(document.getElementsByClassName('chat-message')).forEach(e => e.classList.add('chat-message-chat-opened'))
    }
    ...
    function hideChat () {
      ...
      chat.scrollTop = chat.scrollHeight // Stay bottom of the list
      Array.from(document.getElementsByClassName('chat-message')).forEach(e => e.classList.remove('chat-message-chat-opened'))
    }

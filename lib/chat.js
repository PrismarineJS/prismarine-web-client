...
class ChatBox extends LitElement {
  ...
  init (client, renderer) {
    ...
    function enableChat (isCommand) {
      ...
      document.getElementsByClassName('chat-message').forEach(e => e.classList.add('chat-message-chat-opened'))
    }
    ...
    function hideChat () {
      ...
      chat.scrollTop = chat.scrollHeight // Stay bottom of the list
      document.getElementsByClassName('chat-message').forEach(e => e.classList.remove('chat-message-chat-opened'))
    }

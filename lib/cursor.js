...
class Cursor {
  constructor (viewer, renderer) {
    ...
    bot.lastPlaced = 4 // ticks since last placed
    bot.on('physicsTick', () => { if (bot.lastPlaced < 4) bot.lastPlaced++ })
  }
  ...
  update (bot) {
    ...
    // Place
    if (cursorBlock && this.buttons[2] && (!this.lastButtons[2] || cursorChanged) && bot.lastPlaced >= 4) {
